...
X = np.load('/Users/marton/DTU_MSC/IV_SEMESTER/Thesis/data/5GHz_7UAV/X_rss_data.npy')
y = np.load('/Users/marton/DTU_MSC/IV_SEMESTER/Thesis/data/5GHz_7UAV/y_rss_data.npy')
# Split data with validation
X_temp, X_test, y_temp, y_test = train_test_split(X[1000:], y[1000:], test_size=0.2, random_state=42, shuffle=True)
...
joblib.dump(scaler_X, '/Users/marton/DTU_MSC/IV_SEMESTER/Thesis/data/5GHz_7UAV/X_rss_scaler.joblib')
joblib.dump(scaler_y, '/Users/marton/DTU_MSC/IV_SEMESTER/Thesis/data/5GHz_7UAV/y_rss_scaler.joblib')


# In[18]:


noise_levels = np.arange(0, 51, 5)  # 0 to 50, step 5
results = []
def add_noise(X, noise_level=0.0):
    # X shape: (num_samples, 33), cols 0-11: anchor positions, 12-32: path losses
    num_path_loss_cols = 21  # 21 pairwise path losses for 7 UAVs
    noise = np.zeros(X.shape)
    noise[:, 12:12+num_path_loss_cols] = np.random.normal(0, noise_level, (X.shape[0], num_path_loss_cols))  # Noise for path losses only
    return X + noise
for noise_level in noise_levels:
    print(f"Training DNN and RFR with noise level: {noise_level}")

    # Add noise to train, val, test path losses, then scale
    ...
    y_pred_dnn_scaled = model_dnn.predict(X_test_noisy_scaled, batch_size=32)
    y_pred_dnn = scaler_y.inverse_transform(y_pred_dnn_scaled)
    np.save(f'/Users/marton/DTU_MSC/IV_SEMESTER/Thesis/data/5GHz_7UAV/ground_truth_rss_DNN_y_pred_noise_{noise_level}.npy', y_pred_dnn)
    np.save(f'/Users/marton/DTU_MSC/IV_SEMESTER/Thesis/data/5GHz_7UAV/ground_truth_rss_DNN_y_test_noise_{noise_level}.npy', y_test)

    # RFR
    model_rfr = RandomForestRegressor(n_estimators=200, random_state=42, n_jobs=-1)
    ...
    np.save(f'/Users/marton/DTU_MSC/IV_SEMESTER/Thesis/data/5GHz_7UAV/ground_truth_rss_RFR_y_pred_noise_{noise_level}.npy', y_pred_rfr)
    np.save(f'/Users/marton/DTU_MSC/IV_SEMESTER/Thesis/data/5GHz_7UAV/ground_truth_rss_RFR_y_test_noise_{noise_level}.npy', y_test)
...
# Save DataFrame
results_df.to_csv('/Users/marton/DTU_MSC/IV_SEMESTER/Thesis/data/5GHz_7UAV/rss_results.csv', index=False)


# In[19]:


def plot_dnn_rfr_results_7uav(results_df, save_dir='/Users/marton/DTU_MSC/IV_SEMESTER/Thesis/pictures/ML_methods/7UAV/path_loss'):
...
results_df = pd.read_csv('/Users/marton/DTU_MSC/IV_SEMESTER/Thesis/data/5GHz_7UAV/rss_results.csv')
